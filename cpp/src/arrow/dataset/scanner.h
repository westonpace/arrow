...
/// Scan-specific options, which can be changed between scans of the same dataset.
///
/// A dataset consists of 1-many individual fragments.  A fragment is anything
...
/// schema.  This is typically referred to as the physical schema.  Conversion
/// from the physical schema to the unified schema is a process known as
/// evolution.
struct ARROW_DS_EXPORT ScanV2Options : public compute::ExecNodeOptions {
  ...
  /// \brief Minimum number of rows to read ahead in a fragment
  ///
  ...
  /// then the actual readahead will be +/- 100MiB (per fragment)
  int32_t target_bytes_readahead = kDefaultBytesReadahead;

  /// \brief Number of fragments to read ahead
  ///
  /// The total readahead will be fragment_readahead * target_bytes_readahead.
