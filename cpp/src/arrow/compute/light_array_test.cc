...
//     for (auto offset : {0, 1}) {
//       ARROW_SCOPED_TRACE("Constructor offset: ", offset);
//       std::shared_ptr<Array> array = ArrayFromJSON(null(), "[null, null, null]");
//       array = array->Slice(array_offset);
//       int length = static_cast<int32_t>(array->length()) - offset - array_offset;
//       KeyColumnArray kc_array = ColumnArrayFromArrayData(array->data(), offset,
//       length); ASSERT_EQ(offset + array_offset, kc_array.bit_offset(0));
//       ASSERT_EQ(offset + array_offset, kc_array.bit_offset(1));
//       ASSERT_EQ(array->data()->buffers[0]->data(), kc_array.data(0));
//       ASSERT_EQ(array->data()->buffers[1]->data(), kc_array.data(1));
//       ASSERT_EQ(length, kc_array.length());
...
TEST(KeyColumnArray, FromExecBatch) {
  ...
  ColumnArraysFromExecBatch(batch, &arrays);
  ...
  ColumnArraysFromExecBatch(batch, 1, 1, &arrays);
...
TEST(ExecBatchBuilder, AppendNullsBeyondLimit) {
  ...
  MemoryPool* pool = owned_pool.get();
  {
    ExecBatchBuilder builder;
    int num_appended = 0;
    ASSERT_OK(builder.AppendNulls(pool, {int64(), boolean()}, 10, &num_appended));
    ASSERT_EQ(10, num_appended);
    ASSERT_OK(builder.AppendNulls(pool, {int64(), boolean()}, num_rows_max + 1 - 10,
                                  &num_appended));
    ASSERT_EQ(num_rows_max - 10, num_appended);
    ExecBatch built = builder.Flush();
    ASSERT_EQ(num_rows_max, built.length);
    ASSERT_NE(0, pool->bytes_allocated());
  }
  ASSERT_EQ(0, pool->bytes_allocated());
}
...
TEST(ExecBatchBuilder, AppendValuesBeyondLimit) {
  ...
  std::shared_ptr<Array> trimmed_values = ConstantArrayGenerator::Int32(num_rows_max);
  ExecBatch batch({values}, num_rows_max + 1);
  ExecBatch trimmed_batch({trimmed_values}, num_rows_max);
  ...
  std::iota(second_set_row_ids.begin(), second_set_row_ids.end(), 10);
  {
    ...
    ASSERT_OK(builder.AppendSelected(pool, batch, 10, first_set_row_ids.data(),
                                     ...
                                     /*num_cols=*/1));
    ASSERT_EQ(10, num_appended);
    ASSERT_OK(builder.AppendSelected(pool, batch, num_rows_max - 10 + 1,
                                     second_set_row_ids.data(), &num_appended,
                                     /*num_cols=*/1));
    ASSERT_EQ(num_rows_max - 10, num_appended);
    ExecBatch built = builder.Flush();
