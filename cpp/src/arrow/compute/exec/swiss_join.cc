...
void SwissTableForJoin::UpdateHasMatchForKeys(int64_t thread_id, int num_ids,
  ...
  if (no_duplicate_keys_) {
    UpdateHasMatchForPayloads(thread_id, num_ids, key_ids);
    return;
  }
  ...
  for (int i = 0; i < num_ids; ++i) {
    uint32_t key_id = key_ids[i];

    // Mark all rows with the same key in hash table as having a match
    //
    uint32_t bit_id_begin = row_offset_for_key_[key_id];
    uint32_t bit_id_end = row_offset_for_key_[key_id + 1];
    for (uint32_t bit_id = bit_id_begin; bit_id < bit_id_end; ++bit_id) {
      bit_util::SetBit(bit_vector, bit_id);
    }
  }
}
  ...
  for (int minibatch_start = 0; minibatch_start < num_rows;) {
    ...
    if (join_type_ == JoinType::LEFT_SEMI || join_type_ == JoinType::LEFT_ANTI ||
      ...
      while (match_iterator.GetNextBatch(minibatch_size, &num_matches_next,
        ...
        //
        if (no_duplicate_keys || no_payload_columns) {
...
class SwissJoin : public HashJoinImpl {
 ...
 private:
  ...
  Status ScanTask(size_t thread_id, int64_t task_id) {
    ...
    //
    int bit_to_output = join_type_ == JoinType::RIGHT_SEMI ? 1 : 0;
    ...
    for (int64_t mini_batch_start = start_row; mini_batch_start < end_row;) {
      ...
      // Get the list of payload ids from this mini-batch to output.
      //
      int num_ids = 0;
      util::bit_util::bits_to_indexes(
          bit_to_output, hardware_flags_, static_cast<int>(mini_batch_size_next),
          hash_table_.has_match() + mini_batch_start / 8, &num_ids,
          selection_buf.mutable_data(), static_cast<int>(mini_batch_start % 8));
      for (int i = 0; i < num_ids; ++i) {
        payload_ids_buf.mutable_data()[i] =
            static_cast<uint32_t>(selection_buf.mutable_data()[i] + mini_batch_start);
      }
      if (num_ids > 0) {
        // Map payload ids to corresponding key ids (same key can
        // correspond to a range of payloads).
        //
        hash_table_.payload_ids_to_key_ids(num_ids, payload_ids_buf.mutable_data(),
                                           key_ids_buf.mutable_data());
        ...
        Status status = local_states_[thread_id].materialize.AppendBuildOnly(
            num_ids, key_ids_buf.mutable_data(), payload_ids_buf.mutable_data(),
            [&](ExecBatch batch) {
