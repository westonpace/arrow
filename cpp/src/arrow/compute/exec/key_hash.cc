...
void Hashing32::hash_fixedlen_imp(uint32_t num_rows, uint64_t length, const uint8_t* keys,
  ...
  for (uint32_t i = 0; i < num_rows_safe; ++i) {
    ...
    if (T_COMBINE_HASHES) {
      hashes[i] = combine_hashes_imp(hashes[i], acc);
    } else {
      hashes[i] = acc;
    }
  }
...
void Hashing32::hash_bit_imp(int64_t bit_offset, uint32_t num_keys, const uint8_t* keys,
  ...
  for (uint32_t i = 0; i < num_keys; ++i) {
    ...
    if (T_COMBINE_HASHES) {
      hashes[i] = combine_hashes_imp(hashes[i], hash);
    } else {
      hashes[i] = hash;
    }
  }
...
void Hashing32::hash_int_imp(uint32_t num_keys, const T* keys, uint32_t* hashes) {
  ...
  for (uint32_t ikey = 0; ikey < num_keys; ++ikey) {
    ...
    if (T_COMBINE_HASHES) {
      hashes[ikey] = combine_hashes_imp(hashes[ikey], hash);
    } else {
      hashes[ikey] = hash;
    }
  }
    ...
    for (size_t icol = 0; icol < cols.size(); ++icol) {
      if (cols[icol].metadata().is_null_type) {
        if (icol == 0) {
          ...
          for (uint32_t i = 0; i < batch_size_next; ++i) {
            hashes[first_row + i] = combine_hashes_imp(hashes[first_row + i], 0);
          }
        }
        continue;
      }
      ...
      if (cols[icol].data(0)) {
        if (icol == 0) {
          ...
          for (int i = 0; i < num_null_indices; ++i) {
            hashes[first_row + null_indices[i]] =
                combine_hashes_imp(null_hash_temp[i], 0);
          }
        }
      }
    }
    ...
    for (size_t icol = 0; icol < cols.size(); ++icol) {
      if (cols[icol].metadata().is_null_type) {
        if (icol == 0) {
          ...
          for (uint32_t i = 0; i < batch_size_next; ++i) {
            hashes[first_row + i] = combine_hashes_imp(hashes[first_row + i], 0ULL);
          }
        }
        continue;
      }
      ...
      if (cols[icol].metadata().is_fixed_length) {
        ...
        if (col_width == 0) {
          ...
          hash_fixed(icol > 0, batch_size_next, col_width,
                     cols[icol].data(1) + first_row * col_width, hashes + first_row);
        }
      } else {
        // TODO: add support for 64-bit offsets
        hash_varlen(icol > 0, batch_size_next, cols[icol].offsets() + first_row,
