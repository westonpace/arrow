...
inline __m256i Hashing32::combine_hashes_imp_avx2(__m256i previous_hash, __m256i hash) {
...
void Hashing32::avalanche_all_avx2(uint32_t num_rows_to_process, uint32_t* hashes,
  ...
  for (uint32_t i = 0; i < num_rows_to_process / unroll; ++i) {
    ...
    if (T_COMBINE_HASHES) {
      ...
      acc = combine_hashes_imp_avx2(previous_hash, acc);
    }
  ...
  for (uint32_t i = num_rows_to_process - (num_rows_to_process % unroll);
    ...
    if (T_COMBINE_HASHES) {
      hashes[i] = combine_hashes_imp(hashes[i], avalanche(hashes_temp_for_combine[i]));
    } else {
...
uint32_t Hashing32::hash_fixedlen_imp_avx2(uint32_t num_rows, uint64_t length,
  ...
  avalanche_all_avx2<T_COMBINE_HASHES>(num_rows_to_process, hashes,
